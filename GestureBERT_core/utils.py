...
def openpose_to_smpl(inverse_map):
    # First, create an inverse map: smplx_index → openpose_index
    n_smplx_joints = max(inverse_map) + 1  # or just use known SMPL-X count (e.g., 127)
    map = [None] * n_smplx_joints

    for openpose_idx, smplx_idx in enumerate(inverse_map):
        map[smplx_idx] = openpose_idx

    # Remove None entries if some SMPL-X joints aren't mapped
    op2smplx = np.array(
        [i for i in map if i is not None], dtype=np.int32
    )
    return op2smplx
