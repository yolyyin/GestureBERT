...
def guess_init_masked(model,
                      ...
                      joints_conf=None,
                      **kwargs):
    ''' Initializes the camera translation vector, masking out unreliable joints
        ...
        joints_conf: torch.tensor 1xJ (optional)
            Confidence values for each joint (0 to 1); joints with 0 are ignored
    ...
    # Decode the body pose if using VPoser
    ...
    # Run the model forward to get 3D joints
    ...
    # Optional: joint confidences
    if joints_conf is not None:
        joints_conf = joints_conf.to(device=joints_3d.device)

    diff3d_list, diff2d_list, weights = [], [], []
    print(f"edge_idxs:{edge_idxs}")
    for edge in edge_idxs:
        i, j = edge[0], edge[1]
        if joints_conf is not None:
            conf = joints_conf[0, i] * joints_conf[0, j]
            print(f"conf:{conf}")
            if conf.item() < 1e-8:
                continue  # skip unreliable joints
        else:
            conf = 1.0
        diff3d = joints_3d[:, i] - joints_3d[:, j]
        diff2d = joints_2d[:, i] - joints_2d[:, j]
        diff3d_list.append(diff3d)
        diff2d_list.append(diff2d)
        weights.append(conf)

    if not diff3d_list:
        raise ValueError("No valid joint pairs for initialization due to low confidence.")

    diff3d = torch.stack(diff3d_list, dim=1)
    diff2d = torch.stack(diff2d_list, dim=1)
    weights = torch.tensor(weights, device=joints_3d.device).view(1, -1)

    # Compute weighted lengths
    length_2d = diff2d.pow(2).sum(dim=-1).sqrt()
    length_3d = diff3d.pow(2).sum(dim=-1).sqrt()

    height2d = (length_2d * weights).sum(dim=1) / weights.sum(dim=1)
    height3d = (length_3d * weights).sum(dim=1) / weights.sum(dim=1)

    est_d = focal_length * (height3d / height2d)

    batch_size = joints_3d.shape[0]
    x_coord = torch.zeros([batch_size], device=joints_3d.device, dtype=dtype)
...
class FittingMonitor(object):
    ...
    def create_fitting_closure(self,
                               ...
                               pose_embedding=None,
                               create_graph=False,
                               ref_body_pose=None, visible_joints_mask=None,
                               **kwargs):
        ...
        def fitting_func(backward=True):
            ...
            if ref_body_pose is not None:
                with torch.no_grad():
                    ref_model_output = body_model(return_verts=return_verts,
                                                  body_pose=ref_body_pose,
                                                  return_full_pose=return_full_pose)
            total_loss = loss(body_model_output, camera=camera,
                              ...
                              vposer=vposer,  # by yty
                              ref_model_output=ref_model_output,  # by yty
                              visible_joints_mask=visible_joints_mask,  # by yty
                              **kwargs)
...
class SMPLifyLoss(nn.Module):
    ...
    def forward(self, body_model_output, camera, gt_joints, joints_conf,
                ...
                ref_model_output=None, visible_joints_mask=None, vposer=None, # by yty
                **kwargs):
        ...
        #by yty
        if use_vposer and (ref_model_output is not None) and (visible_joints_mask is not None):
            ref_joints = ref_model_output.joints
            current_joints = body_model_output.joints  # shape [1, 118,3]

            # Create a mask for invisible joints: 1 = keep fixed, 0 = free to move
            # visible_joints_mask shape: [1, 118], values: 1.0 = visible, 0.0 = invisible
            # Expand to 3 values per joint to match [1, 63]
            pose_mask = visible_joints_mask.unsqueeze(-1)  # shape [1, 118,1]

            # inverse mask: 1 = penalize, 0 = ignore
            inverse_mask = 1.0 - pose_mask

            reg_weights=100.0e2 # by yty, important #20.0e2
            pose_reg_loss = torch.sum(((current_joints - ref_joints) ** 2) * inverse_mask) * reg_weights
        else:
            pose_reg_loss = 0.0
        ...
        total_loss = (joint_loss + pprior_loss + shape_loss +
                      ...
                      left_hand_prior_loss + right_hand_prior_loss + pose_reg_loss) # by yty
        return total_loss
