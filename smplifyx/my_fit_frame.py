...
def fit_single_frame(img,
                     ...
                     init_joints_idxs=(3,6,15,16), # by yty
                     use_face=True,
                     use_hands=True,
                     data_weights=None,
    ...
    # by yty
    if not first_frame:
        assert (prev_pose_embedding is not None),"Must provide previous pose embedding if it isn't the 1st frame!"
        assert (prev_camera_trans is not None), "Must provide previous camera translation if it isn't the 1st frame!"
        assert (prev_body_global_orient is not None), "Must provide previous body orientation if it isn't the 1st frame!"
        assert (prev_betas is not None), "Must provide previous body shape if it isn't the 1st frame!"

    use_vposer = kwargs.get('use_vposer', True)
    vposer, pose_embedding = [None, ] * 2
    if use_vposer:
        # by yty initialize with previous pose embedding if not first frame
        pose_embedding = torch.zeros([batch_size, 32],
                                     dtype=dtype, device=device,
                                     requires_grad=True) if first_frame \
            else torch.tensor(prev_pose_embedding,dtype=dtype, device=device,
                                     requires_grad=True)
    ...
    if use_vposer:
        if first_frame:
            ...
            # use previous body pose, by yty
            body_mean_pose = torch.tensor(prev_pose_embedding,dtype=dtype)
    else:
    ...
    print(f"init_joints_idxs:{init_joints_idxs}")
    ...
    if first_frame:
        ...
        init_t = torch.tensor(prev_camera_trans,device=device,dtype=dtype)

    # by yty
    #init_t = fitting.guess_init_masked(body_model, gt_joints, edge_indices,
    ...
    with fitting.FittingMonitor(
        ...
        if first_frame:
            ...
            prev_orient=torch.tensor(prev_body_global_orient,device=device,dtype=dtype)
            new_params = defaultdict(body_pose=body_mean_pose,
                                     global_orient=prev_orient)
        ...
        # by yty, only optimize the camera translation and body orient if it is the first frame
        if first_frame:
            ...
            fit_camera = monitor.create_fitting_closure(
                ...
                #joints_conf=joints_conf, # by yty
            )
        ...
        # close then rotate the body by 180 degrees and also fit to that
        # orientation
        if try_both_orient:
        ...
        for or_idx, orient in enumerate(tqdm(orientations, desc='Orientation')):
            ...
            if first_frame:
                ...
                prev_betas_tensor = torch.tensor(prev_betas, device=device, dtype=dtype)
                new_params = defaultdict(global_orient=orient,
                                         body_pose=body_mean_pose,
                                         betas = prev_betas_tensor)
            body_model.reset_params(**new_params)
            # by yty, only optimize body orient and betas if it is not the first frame
            if not first_frame:
            ...
            if use_vposer and first_frame: # by yty, only make pose_embedding zero in the first frame
                with torch.no_grad():
            ...
            for opt_idx, curr_weights in enumerate(tqdm(opt_weights, desc='Stage')):
                ...
                # by yty
                ref_body_pose = None
                visible_joints_mask = (joints_conf > 0).float()
                if use_vposer:
                    final_params.append(pose_embedding)
                    with torch.no_grad():
                        ref_body_pose = vposer.decode(pose_embedding.detach().clone()).get('pose_body')
                ...
                closure = monitor.create_fitting_closure(
                    ...
                    ref_body_pose=ref_body_pose, visible_joints_mask=visible_joints_mask,  # by yty
                )
    ...
    body_pose_raw = (vposer.decode(pose_embedding).get('pose_body')) if use_vposer else None

    if save_meshes or visualize:
        body_pose = body_pose_raw.reshape(1, -1) if use_vposer else None
    ...
    return results[min_idx]['result'], body_pose_raw.detach().cpu().numpy(), visible_joints_mask.detach().cpu().numpy(), final_loss_val
